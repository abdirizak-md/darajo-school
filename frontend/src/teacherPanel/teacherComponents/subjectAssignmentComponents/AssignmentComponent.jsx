...
const AssignmentComponent = ({ setAddAssignment }) => {
  const statusStyles = {
          ...
          <tbody className="table-row-group border-inherit">
            ...
            {assignments.map((assignment) => (
              ...
              >
                ...
                <td className="p-4">
                  <span
                    className={`py-1 px-2.5 rounded-2xl text-sm font-medium ${statusStyles[assignment.status?.toLowerCase()] || "bg-gray-100 text-gray-500"}`}
                  >
                    {" "}
                    {assignment.status}
                  </span>
                ...
                <td className="p-4">
                  <div className="flex gap-2">
                    <button
                      type="button"
                      className="bg-[#10b981] hover:bg-[#0c9c6c] text-white border border-[#e1e5e9] px-4 py-1.5 rounded-md"
                    >
                      View
                    </button>
                    <button
