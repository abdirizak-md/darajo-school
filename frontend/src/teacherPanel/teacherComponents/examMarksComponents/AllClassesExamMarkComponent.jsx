          ...
          <tbody className="table-row-group border-inherit">
            {examMarks?.data.length === 0 && (
              <tr>
                <td
                  colSpan="8"
                  className="p-4 border-b border-[#e1e5e9] text-center text-[#666]"
                >
                  No Exam Marks Found
                </td>
              </tr>
            ...
            {examMarks?.data.map((exam, index) => (
              <tr key={index} className="hover:bg-[#f8f9fa]">
                ...
                <td className="p-4 border-b border-[#e1e5e9] text-left capitalize">
                  <span
                    className={`py-1 px-2.5 rounded-2xl text-sm font-medium  ${exam.status === "Active" ? "bg-[#d1fae5] text-[#10b981]" : ""} ${exam.status === "Pending" ? " bg-[#fef3c7] text-[#92400e]" : ""} ${exam.status === "Completed" ? " bg-[#dbeafe] text-[#1e40af]" : ""}`}
                  >
                    {exam.status}
                  </span>
                </td>
                <td className="p-4 border-b border-[#e1e5e9] text-left flex gap-2">
                  <button className="bg-[#f8f9fa] hover:bg-[#ffffff] text-[#333] border border-[#e1e5e9] px-4 py-2 rounded-md">
                    View
                  </button>
                  <button
                    className="bg-[#fcd116] hover:bg-[#ffda33] text-[#333] border border-[#e1e5e9] px-4 py-2 rounded-md"
                    onClick={() => startUpdatingIndex(index)}
                  >
                    Edit
                  </button>
                  <button
...
export default AllClassesExamMarkComponent;
