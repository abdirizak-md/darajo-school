  ...
  return (
    <div className="bg-white p-6 mb-6 shadow rounded-md">
        <div className="flex justify-between items-center mb-4">
            <span className='text-[#333] lg:text-2xl font-bold'>Daily Attendance</span>
            <button className='lg:px-5 px-3 py-2 cursor-pointer text-white bg-orange-500 rounded-md inline-flex items-center gap-2'><FaPlus />Take Daily Attendance</button>
        </div>
        
        <form className="grid grid-cols-1 lg:grid-cols-[3fr_150px] md:grid-cols-[3fr_2fr_150px] gap-3 ">
            <div className="">
                <input type="text" placeholder="Search daily attendance report" className="lg:w-full w-full p-2.5 border border-[#e1e5e9] rounded-md text-lg transition-all duration-300 ease-in-out placeholder:text-sm"/>
            </div>
        ...
        <div className="w-full overflow-x-auto">
            <table className="min-w-200 w-full border-collapse mt-4">
                ...
                <tbody className='table-row-group border-inherit'>
                    ...
                    {   dailyAttendance.map((dailyAttend) => (
                        <tr key={dailyAttend.id} className="hover:bg-[#f8f9fa] border-b border-[#e1e5e9] text-left">
                        <td className='p-4'>{dailyAttend.assignmetTitle}</td>
                        <td className='p-4'>{dailyAttend.subject}</td>
                        <td className='p-4'>{dailyAttend.classLevel}</td>
                        <td className='p-4'><span className={`py-1 px-2.5 rounded-2xl text-sm font-medium ${ statusStyles[dailyAttend.status?.toLowerCase()] || "bg-gray-100 text-gray-500"  }`}> {dailyAttend.status}</span></td>
                        <td className='p-4'>
