...
import { useGetExamsQuery } from '../../../redux/features/exam';
import { useSaveMarksMutation } from '../../../redux/features/entryMark';

import { useState, useMemo } from 'react';

const EntryExamMarkComponent = () => {
  ...
  const allowedSections = useMemo(() => {
    ...
    assignmentList.forEach(a => {
      if (String(a?.classId?._id) === String(selectedClass)) {
        if (a?.sectionId?._id) {
          map.set(a.sectionId._id, a.sectionId);
        }
      }
    });

    return Array.from(map.values());
  }, [assignmentList, selectedClass]);
  ...
  const allowedSubjects = useMemo(() => {
    ...
    assignmentList.forEach(a => {
      if (
        String(a?.classId?._id) === String(selectedClass) &&
        String(a?.sectionId?._id) === String(selectedSection)
      ) {
        if (a?.subjectId?._id) {
          map.set(a.subjectId._id, a.subjectId);
        }
      }
    });

    return Array.from(map.values());
  }, [assignmentList, selectedClass, selectedSection]);
  ...
  const filteredExams = useMemo(() => {
    ...
    return examList.filter(e =>
      String(e.classId?._id || e.classId) === String(selectedClass) &&
      String(e.sectionId?._id || e.sectionId) === String(selectedSection) &&
      String(e.subjectId?._id || e.subjectId) === String(selectedSubject)
    );
  }, [examList, selectedClass, selectedSection, selectedSubject]);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!selectedExamId || !selectedSubject || !selectedClass || !selectedSection) {
      alert("Please select all fields");
      return;
    }

    const marksList = Object.entries(marks).map(([id, val]) => ({
      studentId: id,
  ...
  return (
    <div className="bg-white p-6 mb-8 shadow rounded-md">
      ...
      {/* FILTER */}
      <form className="grid grid-cols-1 lg:grid-cols-[2fr_2fr_2fr_2fr] gap-5">
      ...
      <form onSubmit={handleSubmit} className="overflow-x-auto">
        ...
        <table className="w-full mt-4 border-collapse">
          <thead>
            <tr>
              <th className="p-4 bg-[#f8f9fa]">Student</th>
              <th className="p-4 bg-[#f8f9fa]">Admission</th>
              <th className="p-4 bg-[#f8f9fa]">Marks</th>
            </tr>
          </thead>
          ...
          <tbody>
            {studentList.map(s => (
              <tr key={s._id}>
                ...
                <td className="p-4">{s.admissionNumber}</td>
                <td className="p-4">
                  <input
...
// export default EntryExamMarkComponent;  import { IoSearchOutline } from 'react-icons/io5'
// import { useGetAssignmentsQuery } from '../../../redux/features/assignModule'
