...
const TeacherPanelLessonPlansPage = () => {
  ...
  return (
    <section className='max-w-400 mx-auto p-8 bg-[#f5f7fa] h-screen overflow-y-auto custom-scrollbar'>
        <Link to='/teacher' className="flex justify-center w-fit items-center cursor-default p-3 rounded-md bg-orange-500 mb-8 transition-all duration-300 hover:-translate-y-1 hover:shadow-md active:scale-95">
            <MdArrowBackIos  className='text-white' size={24}/>
            <span className='text-white font-medium'>Back to Dashboard</span>
        </Link>
    
        <div className="bg-white rounded-md p-8 shadow-[0_5px_20px_rgba(0,0,0,0.1)] mb-8">
            <h1 className='text-orange-500 lg:text-4xl text-2xl font-bold mb-4'>🏠 Lesson Plans</h1>
        ...
        <div className="lg:flex md:flex grid grid-cols-[repeat(auto-fit,minmax(100px,1fr))] gap-4 mb-4">
            <button onClick={() => setActive('plans')} className={`px-8 py-3 border border-[#e1e5e9]  rounded-md shadow-[0_5px_20px_rgba(0,0,0,0.1)] ${active == 'plans' ? 'bg-orange-500 text-white' : 'bg-white'}`}>All Plans</button>
            <button onClick={() => setActive('maths')} className={`px-8 py-3 border border-[#e1e5e9] rounded-md shadow-[0_5px_20px_rgba(0,0,0,0.1)] ${active == 'maths' ? 'bg-orange-500 text-white' : 'bg-white'}`}>Mathematics</button>
            <button onClick={() => setActive('English')} className={`px-8 py-3 border border-[#e1e5e9]  rounded-md shadow-[0_5px_20px_rgba(0,0,0,0.1)] ${active == 'English' ? 'bg-orange-500 text-white' : 'bg-white'}`}>English</button>
            <button onClick={() => setActive('Physics')} className={`px-8 py-3 border border-[#e1e5e9]  rounded-md shadow-[0_5px_20px_rgba(0,0,0,0.1)] ${active == 'Physics' ? 'bg-orange-500 text-white' : 'bg-white'}`}>Physics</button>
            <button onClick={() => setActive('Biology')} className={`px-8 py-3 border border-[#e1e5e9]  rounded-md shadow-[0_5px_20px_rgba(0,0,0,0.1)] ${active == 'Biology' ? 'bg-orange-500 text-white' : 'bg-white'}`}>Biology</button>
        </div>
...
export default TeacherPanelLessonPlansPage