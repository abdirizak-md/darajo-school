...
import OverDueFeeComponent from "../parentComponents/feeCollectionComponents/OverDueFeeComponent";
import PendingFeesComponent from "../parentComponents/feeCollectionComponents/PendingFeesComponent";
import RecentPaymentComponent from "../parentComponents/feeCollectionComponents/RecentPaymentComponent";

const ParentPanelFeesCollectionPage = () => {
  ...
  return (
    <section className="max-w-400 mx-auto p-8 bg-[#f5f7fa] h-screen overflow-y-auto custom-scrollbar">
      ...
      <div className="flex gap-5 mb-8">
        {["recent", "pending", "overdue"].map((tab, index) => (
          <button
            key={index}
            onClick={() => setActive(tab)}
            className={`px-8 py-3 border border-[#e1e5e9]  rounded-md shadow-[0_5px_20px_rgba(0,0,0,0.1)] ${active == tab ? "bg-orange-500 text-white" : "bg-white"}`}
          >
...
export default ParentPanelFeesCollectionPage;
