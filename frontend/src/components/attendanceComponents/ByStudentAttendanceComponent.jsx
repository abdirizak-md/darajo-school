  ...
  return (
    <div className="bg-white p-6 mb-6 shadow rounded-md">
        <div className="flex justify-between items-center mb-4">
            <span className='text-[#333] lg:text-2xl font-bold'>By Student Attendance</span>
            <button className='lg:px-5 px-3 py-2 cursor-pointer text-white bg-orange-500 rounded-md inline-flex items-center gap-2'><FaPlus />Take Student Attendance</button>
        </div>
        
        <form className="grid grid-cols-1 lg:grid-cols-[3fr_150px] md:grid-cols-[3fr_2fr_150px] gap-3 ">
            <div className="">
                <input type="text" placeholder="Search student attendances" className="lg:w-full w-full p-2.5 border border-[#e1e5e9] rounded-md text-lg transition-all duration-300 ease-in-out placeholder:text-sm"/>
            </div>
        ...
        <div className="w-full overflow-x-auto">
            <table className="min-w-200 w-full border-collapse mt-4">
                ...
                <tbody className='table-row-group border-inherit'>
                    ...
                    {   studentAttendance.map((studentAttend) => (
                        <tr key={studentAttend.id} className="hover:bg-[#f8f9fa] border-b border-[#e1e5e9] text-left">
                        <td className='p-4'>{studentAttend.assignmetTitle}</td>
                        <td className='p-4'>{studentAttend.subject}</td>
                        <td className='p-4'>{studentAttend.classLevel}</td>
                        <td className='p-4'><span className={`py-1 px-2.5 rounded-2xl text-sm font-medium ${ statusStyles[studentAttend.status?.toLowerCase()] || "bg-gray-100 text-gray-500"  }`}> {studentAttend.status}</span></td>
                        <td className='p-4'>
