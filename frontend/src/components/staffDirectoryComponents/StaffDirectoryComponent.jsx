            ...
            {teachers?.map((teacher, index) => (
              <tr key={index} className="hover:bg-[#f8f9fa]">
                ...
                <td className="p-4 border-b border-[#e1e5e9] text-left capitalize">
                  <span
                    className={`py-1 px-2.5 rounded-2xl text-sm font-medium  ${teacher.status === "Active" ? "bg-[#d1fae5] text-[#10b981]" : ""} ${teacher.status === "Pending" ? " bg-[#fef3c7] text-[#92400e]" : ""} ${teacher.status === "Completed" ? " bg-[#dbeafe] text-[#1e40af]" : ""}`}
                  >
                    {teacher.status}
                  </span>
                </td>
